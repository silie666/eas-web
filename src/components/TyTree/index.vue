...
import { TyTreeDropdownItems, TyTreeParentDropdownItems } from "@/components/TyTree/types";
interface Tree {
  id: number;
  name: string;
  highlight?: boolean;
...
const props = withDefaults(
  defineProps<{
    ...
    idKey: string;
    parentIdKey: string;
    parentDropdownItems: TyTreeParentDropdownItems[];
    dropdownItems: TyTreeDropdownItems[];
  }>(),
  {
    treeLoading: false,
    treeData: () => [],
...
<template>
  ...
  >
    <div class="flex items-center h-[34px]">
      <el-input class="ml-2" size="small" v-model="searchValue" :placeholder="$t('请输入关键字搜索')" clearable>
        <template #suffix>
        ...
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item>
              ...
              >
                {{ isExpand ? $t("全部折叠") : $t("全部展开") }}
              </el-button>
            </el-dropdown-item>
            <el-dropdown-item v-for="item in parentDropdownItems" :key="item.title">
              <el-button
                v-auth="item.auth"
                :class="buttonClass"
                link
    ...
    >
      <template #default="{ node, data }">
        ...
        >
          ...
          {{ node.label }}
          <el-dropdown v-if="dropdownItems.length > 0">
            <IconifyIconOffline class="w-[28px] cursor-pointer" width="18px" :icon="More2Fill" />
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item v-for="item in dropdownItems" :key="item.title">
                  <el-button
                    ...
                    type="primary"
                    :icon="item.icon"
                    @click="item.onClick(data[idKey], data['name'])"
                  >
                    {{ item.title }}
                  </el-button>
                </el-dropdown-item>
