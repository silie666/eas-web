...
export default defineComponent({
  ...
  setup(props, { emit, slots, attrs }) {
    ...
    // 写一个存储用户习惯的列设置 by汤圆大哥
    const route = useRoute();
    dynamicColumns.value.map(column => {
      if (checkedColumns.value.includes(column.label)) {
        column.hide = false;
        column.hidden = false;
      } else {
    ...
    function handleCheckColumnListChange(val: boolean, label: string) {
      ...
      dynamicColumns.value.filter(item => item.label === label)[0].hidden = !val;
    }
    ...
    // 新增判断列设置宽度
    const dynamicWidth = ref("160");
    for (const column of props.columns) {
      if (column.label && column.label.length > 6) {
        dynamicWidth.value = "200";
        break;
      }
    }
                      ...
                      {checkColumnList.map(item => {
                        return (
                          <div class="flex items-center">
                            ...
                            >
                              <span title={item} class="inline-block w-[140px] truncate hover:text-text_color_primary">
                                {item}
                              </span>
                            </el-checkbox>
                          </div>
