...
declare global {
  ...
  interface ViteEnv {
    ...
    VITE_BACK_END_DEV_URL: string;
  }

  /**
   * 自定义一个汤圆搜索列字段
   */
  interface TySearchColumns<T = Record<string, any>> {
    search?: {
      type: "input" | "dateRange" | "select" | "inputRange" | "radio"; // 搜索框类型
      key?: keyof T; // 对应后端的键特殊处理
      options?: any; // 给那些下拉框用的数据
      multiple?: boolean; // 给那些下拉框用的数据
    };
  }
  /**
   *  继承 `@pureadmin/table` 的 `TableColumns` ，方便全局直接调用
   */
  interface TableColumnList<T = Record<string, any>> extends Array<TableColumns & TySearchColumns<T>> {}
